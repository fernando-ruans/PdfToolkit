...
export default function Merge() {
  ...
  const onBrowse = (e) => setFiles(Array.from(e.target.files))
  ...
  <div className="max-w-xl mx-auto mt-10 p-8 bg-white rounded-2xl shadow-xl border border-blue-100">
      ...
      <input type="file" multiple accept="application/pdf" ref={inputRef} className="hidden" onChange={onBrowse} />
      <button className="px-3 py-2 bg-blue-600 text-white rounded flex items-center gap-2 hover:bg-blue-700 transition" onClick={() => inputRef.current?.click()}>
        <FaLayerGroup className="animate-pulse" /> Selecionar PDFs
      </button>
      {files.length > 0 && (
        <div className="mt-4">
          ...
          <button className="mt-4 px-3 py-2 bg-green-600 text-white rounded flex items-center gap-2 hover:bg-green-700 transition disabled:opacity-50" onClick={merge} disabled={!files.length}>
